...
{
    public class MainRepositoryBase<TDbContext> : IDisposable, IAsyncDisposable where TDbContext : DbContext
    {
        public TDbContext Context { get; private set; }
        private List<RepositoryBase<TDbContext>> Repositories { get; } = new();

        public MainRepositoryBase(IDbContextFactory<TDbContext> dbFactory)
        {
        ...
        protected TRepository GetOrCreateRepository<TRepository>() where TRepository : RepositoryBase<TDbContext>
        {
